 ...
 */
static inline uint16_t mavlink_msg_mission_item_reached_pack(uint8_t system_id,
...
static inline uint16_t mavlink_msg_mission_item_reached_pack_chan(
		...
		mavlink_message_t* msg, uint16_t seq) {
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[2];
	_mav_put_uint16_t(buf, 0, seq);
 ...
 * @return Sequence
 */
static inline uint16_t mavlink_msg_mission_item_reached_get_seq(
 ...
 */
static inline void mavlink_msg_mission_item_reached_decode(
		const mavlink_message_t* msg,
